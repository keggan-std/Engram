...
| **F4** — `searchAll()` skips `checkPermission()` | **CONFIRMED → FIXED 2026-08-07, `cc138b6`** | Was: [`cross-instance.service.ts:493`](../src/services/cross-instance.service.ts#L493) reached `` `SELECT * FROM ${scope}` `` gated only by the *foreign* instance's self-reported `sharing_types`. Since that array comes from `~/.engram/instances.json` — a machine-global, unsigned, any-local-process-writable file — the gate was attacker-influenceable. This was worse than "a whitelist gap." **Reproduced before the fix** against the built service: `checkPermission('observations')` threw while `searchAll(scope:'observations')` returned the row. `searchAll` now delegates per-instance authorization to `checkPermission`, validates `scope` against `QUERYABLE_TABLES` once up front, and uses the resolved `db_path`; `setSharing` refuses an unshareable type. Bound by `tests/security/cross-instance-scope.test.ts`, which asserts the two readers AGREE for every scope rather than listing blocked names. |
...
| 6 | ~~**P1**~~ **DONE `cc138b6`** | Route `searchAll()` through `checkPermission()`; constrain `sharing_types`. Done, with one deliberate departure: the constraint is enforced in `setSharing` rather than as `z.enum([...QUERYABLE_TABLES])` on the tool schema. Schema-level validation would cover only the MCP path; the service-level check also covers the HTTP route and every internal caller, and is strictly broader. `assertKnownTable()` was unnecessary once `scope` is validated once at the top of `searchAll` — the whitelist is what makes the surviving identifier interpolation safe. | S | F4 |
