...
const conventions = verbosity === "nano"    ? capConventions(0)
                  : verbosity === "minimal" ? capConventions(5)
                  : verbosity === "summary" ? capConventions(10)
...
const universalMode = args.includes("--mode=universal") || 
...
**AR-06 in `find.ts`:** The `AGENT_RULES` constant was updated. Rule AR-06 now explicitly states: *"ALWAYS include `executive_summary` when calling `set_file_notes` or `set_file_notes_batch`. It is required, not optional."*
